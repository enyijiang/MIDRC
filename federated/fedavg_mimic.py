...
def load_data(args, domains, transform):
    ...
    for mode in ['train', 'test']:
        for domain in domains:
            if mode == 'train':
                if domain != target:
                    train_dataset = MimicDataset(os.path.join(constants.MIMIC_DATA_DIR, f'mimic_table_{domain}_{mode}.csv'), args.data_aug_times, transform[mode], n_samples=args.n_source_samples)
                else:
...
if __name__ == '__main__':
    ...
    domains = ['Asian', 'Hispanic', 'White', 'Black']
    ...
    for idx, server in enumerate(domains):
        ...
        # # print(len(clients_dls['train']), len(clients_dls['test']))
        # # for the server 
        # for mode in ['train', 'test']:
        #     if mode == 'test':
