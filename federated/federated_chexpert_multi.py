...
from datasets.CheXpert import ChexpertMLTDataset, ChexpertDataset
...
def load_data(args, domains, transform):
    ...
    for mode in ['train', 'test']:
        for domain in domains:
            ...
            # else:
            if mode == 'train':
                train_dataset = ChexpertMLTDataset(os.path.join(constants.CHEXPERT_DATA_DIR, f'chexpert_table_{domain}_{mode}.csv'), args.data_aug_times, transform[mode], n_samples=args.n_source_samples)
            else:
                train_dataset = ChexpertMLTDataset(os.path.join(constants.CHEXPERT_DATA_DIR, f'chexpert_table_{domain}_{mode}.csv'), args.data_aug_times, transform[mode], n_samples=int(args.n_source_samples//4))
...
def update_global(args, local_models_dict, old_global_model_dict, finetune_global_model_dict, cos_sim, clients_size, clients_size_frac):
    ...
    for key in ret_dict.keys():
        if ret_dict[key].shape != torch.Size([]) and 'fc_out.1' not in key and 'fc_out.2' not in key:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='fl_chexpert')
    ...
    parser.add_argument('--n_source_samples', type=int, default=2000)
    parser.add_argument('--n_target_samples', type=int, default=500)
    ...
    transform = {
     ...
     # ['IL', 'NC', 'CA', 'IN', 'TX']
    domains = ['Edema', 'LungLesion', 'Pneumonia', 'Atelectasis', 'Consolidation']
    # states = ['NC', 'IL']
    # states = ['IL', 'CA', 'IN', 'TX']
    ...
    for idx, server in enumerate(domains):
        ...
        global_model = ResnetMultiTaskNet(resnet=args.resnet, hidden_size=args.hidden_size, num_classes=[1,2,9,2])
        global_model.to(device)
        local_models = [ResnetMultiTaskNet(resnet=args.resnet, hidden_size=args.hidden_size, num_classes=[1,2,9,2]) for _ in range(num_clients)]
