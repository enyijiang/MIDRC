...
def load_clients_data(args, states, transform):
    ...
    for mode in ['train', 'test']:
        for state in states:
            if mode == 'test':
                test_dataset = MidrcMLTDataset(os.path.join(constants.STATE_DATA_DIR, f'MIDRC_table_{state}_{mode}.csv'), args.data_aug_times, transform[mode], 100)
                dl = torch.utils.data.DataLoader(test_dataset, batch_size=args.source_batch_size, shuffle=False, drop_last=args.no_drop_last)
            else:
                train_dataset = MidrcMLTDataset(os.path.join(constants.STATE_DATA_DIR, f'MIDRC_table_{state}_{mode}.csv'), args.data_aug_times, transform[mode], 400)
...
def load_server_data(args, state, transform):
    dls = {'train':[], 'test':[]}
    for mode in ['train', 'test']:
        train_dataset = MidrcMLTDataset(os.path.join(constants.STATE_DATA_DIR, f'MIDRC_table_{state}_{mode}.csv'), args.data_aug_times, transform[mode])
        if mode == 'test':
...
def train(args, da_phase, model, criterions, train_dl):
    global device
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(unlabel_dl, unit='batch') as tepoch:
            for sample in tepoch:
                ...
                output_unlabeled = output_unlabeled[0]              
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='fl/self_train_multi')
    ...
    global_model = ResnetMultiTaskNet(resnet=args.resnet, hidden_size=args.hidden_size)
    local_models = [ResnetMultiTaskNet(resnet=args.resnet, hidden_size=args.hidden_size) for _ in range(num_clients)]
    ...
    for i in range(args.num_global_epochs):
        ...
        for idx in range(num_clients):
            ...
            local_models[idx], (loss, acc, auc) = train(args, 'source', local_models[idx], criterions, clients_dls['train'][idx])
        ...
        global_model, (loss, acc, auc) = self_train(args, 'target', global_model, criterion_covid, server_dls['train'][0], unlabel_dl)
        # print(loss, acc, auc)
        ...
        for idx in range(num_clients): 
            (loss, acc, auc) = test(args, local_models[idx], criterion_covid, clients_dls['test'][idx])
        ...
        (loss, acc, auc) = test(args, global_model, criterion_covid, server_dls['test'][0])
