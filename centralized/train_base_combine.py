...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='base_mix')
    ...
    target_train = MidrcDataset(constants.TARGET_TRAIN_CSV_PATH, args.data_aug_times, 
                                       transform['train'])
    source_train = MidrcDataset(constants.SOURCE_TRAIN_CSV_PATH, args.data_aug_times, 
                                          transform['train'])
    target_test = MidrcDataset(constants.TARGET_TEST_CSV_PATH, 1, 
                                      transform['test'])
    source_test = MidrcDataset(constants.SOURCE_TEST_CSV_PATH, 1, 
                                         transform['test'])
    ...
    subset_1 = torch.utils.data.Subset(source_train, subset_idx)

    subset_idx = get_subset_indices(target_train, args.n_source_samples,
                                    args.data_sampler_seed)
    subset_2 = torch.utils.data.Subset(source_train, subset_idx)

    subset = torch.utils.data.ConcatDataset([subset_1, subset_2])
