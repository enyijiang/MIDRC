...
def train(args: argparse.Namespace, da_phase: str, model: torch.nn.Module,
          ...
          scheduler: torch.optim.lr_scheduler, dataloaders: dict, device: str, finetuneing: bool):
    """
        ...
        for phase in ['train', 'valid']:
            ...
            with tqdm(dataloaders[phase], unit="batch") as tepoch:
                for batch_idx, sample in enumerate(tepoch):
                    ...
                    # labels = sample['covid'].unsqueeze(1).to(device)
                    # races = sample['race'].to(device)
                    # genders = sample['sex'].to(device)
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        # else:
                        #     loss = criterion_covid(probs, labels) + criterion_race(pred_races, races)
                        # implement balance loss task weighting
                        if phase == 'train' and not finetuneing:
                            ...
                            for idx, c in enumerate(criterions[1:]):
                                ...
                                loss = loss + cur_loss * ((cur_loss.clone() / loss_base)**args.alpha) 
                        else:
                            loss = criterions[0](probs, labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='multi_balance_loss')
    ...
    parser.add_argument('--alpha', type=float, default=0.5) # the hyperparameter for weight task loss
    ...
    target_train = MidrcMLTDataset(constants.TARGET_TRAIN_CSV_PATH, args.data_aug_times,
                                       transform['train'])
    source_train = MidrcMLTDataset(constants.SOURCE_TRAIN_CSV_PATH, args.data_aug_times, 
                                          transform['train'])
    target_test = MidrcMLTDataset(constants.TARGET_TEST_CSV_PATH, 1, 
                                      transform['test'])
    source_test = MidrcMLTDataset(constants.SOURCE_TEST_CSV_PATH, 1, 
                                         transform['test'])

    # Define classifier, criterion, and optimizer
    model = ResnetMultiTaskNet(resnet=args.resnet, hidden_size=128)
    # model = MLTCNN()
