...
from datasets.CheXpert import ChexpertMLTDataset
        ...
        for phase in ['train', 'valid']:
            ...
            with tqdm(dataloaders[phase], unit="batch") as tepoch:
                for imgs, targets in tepoch:
                    ...
                    # else:
                        # labels = targets.unsqueeze(1).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='chex2mimic')
    ...
    source_train = ChexpertMLTDataset(constants.PNEU_TRAIN_CSV_PATH, args.data_aug_times, 
                                          transform['train'], n_samples=args.n_source_samples)
    source_test = ChexpertMLTDataset(constants.PNEU_TEST_CSV_PATH, 1, 
    ...
    target_train = ChexpertMLTDataset(constants.EDEMA_TRAIN_CSV_PATH, args.data_aug_times, 
                                          transform['train'], n_samples=args.n_target_samples)
    target_test = ChexpertMLTDataset(constants.EDEMA_TEST_CSV_PATH, args.data_aug_times, 
                                          transform['test'], n_samples=5000)

    # Define classifier, criterion, and optimizer
    model = ResnetMultiTaskNet(resnet=args.resnet, hidden_size=args.hidden_size, num_classes=[1, 2, 10, 2])
    # model = MLTCNN()
