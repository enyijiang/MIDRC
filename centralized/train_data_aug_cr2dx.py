...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='baseline')
    ...
    parser.add_argument('--source_lr', type=float, default=0.00005)
    parser.add_argument('--target_lr', type=float, default=0.0001)
    ...
    target_train = MidrcDataset(constants.TARGET_TRAIN_CSV_PATH,
                                       transform['train'])
    source_train = MidrcDataset(constants.SOURCE_TRAIN_CSV_PATH,
                                          transform['train'])
    target_test = MidrcDataset(constants.TARGET_TEST_CSV_PATH,
                                      transform['test'])
    source_test = MidrcDataset(constants.SOURCE_TEST_CSV_PATH,
                                         transform['test'])

    # Define classifier, criterion, and optimizer
    model = ResNetClassifier(resnet=args.resnet)
    ...
    train_dataset, valid_dataset = torch.utils.data.random_split(
        subset,
        [int(np.floor(0.9 * len(subset))), int(np.ceil(0.1 * len(subset)))],
